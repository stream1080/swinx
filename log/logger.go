...
func NewLogger(out io.Writer, prefix string, flag int) *Logger {
	...
	//  debug 默认打开， calledDepth 深度为 2 , Logger 对象调用日志打印方法最多调用两层到达 output 函数
	zlog := &Logger{
		out:        out,
		prefix:     prefix,
		flag:       flag,
		...
		if log.flag&(BitShortFile|BitLongFile) != 0 {
			...
			if log.flag&BitShortFile != 0 {
				...
				for i := len(file) - 1; i > 0; i-- {
					if file[i] == '/' {
						//找到最后一个'/'之后的文件名称  如:/home/go/src/swinx.go 得到 "swinx.go"
						short = file[i+1:]
						break
					}
				}
