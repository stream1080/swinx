...
type MsgHandle struct {
	...
	TaskQueue      []chan face.Request    // Worker 负责取任务的消息队列
}
...
func NewMsgHandle() *MsgHandle {
	...
	return &MsgHandle{
		...
		TaskQueue:      make([]chan face.Request, poolSize),
	}
}
...
func (h *MsgHandle) StartWorkerPool() {
	...
	for i := 0; i < int(h.WorkerPoolSize); i++ {
		...
		h.TaskQueue[i] = make(chan face.Request, conf.ServerConfig.MaxWorkerTaskSize)
		// 启动当前Worker，阻塞的等待对应的任务队列是否有消息传递进来
		go h.StartWorker(i, h.TaskQueue[i])
	}
}

func (h *MsgHandle) StartWorker(workerId int, taskQueue chan face.Request) {
...
func (h *MsgHandle) SendMsgTaskQueue(request face.Request) {
